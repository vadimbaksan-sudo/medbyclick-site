      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        {categories.map((cat) => (
          <div key={cat} className="mb-14">
            <h2 className="text-lg font-bold text-stone-900 mb-6 flex items-center gap-3">
              {cat}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {courses.filter((c) => c.category === cat).map((course) => (
                <div
                  key={course.id}
                  className="border border-stone-200 rounded-2xl p-6 bg-white hover:border-amber-300 hover:shadow-sm transition-all flex flex-col"
                >
                  <p className="text-xs text-amber-600 font-medium mb-2">{course.category}</p>
                  <h3 className="font-semibold text-stone-900 mb-2 leading-tight">{course.title}</h3>
                  <p className="text-xs text-stone-500 leading-relaxed flex-1 mb-4">{course.description}</p>
      ...
      <div className="bg-amber-50 border-t border-amber-100 py-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link
              href="/ai-diagnostics/"
              className="inline-flex items-center justify-center px-6 py-3 border border-stone-300 hover:border-stone-400 text-stone-700 rounded-lg text-sm transition-colors"
            >
              Try AI diagnostics
            </Link>
          </div>
