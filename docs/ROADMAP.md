...
| Module | Real-functionality definition | Status (2026-07-04) | Depends on | Consult |
|---|---|---|---|---|
| `medconnect` | Real doctor profiles, real booking/matching workflow, replacing `mockDoctors`-style data. | **Booking lifecycle: code done.** Auth-gated Server Action → real `bookings` table (`status`: requested/confirmed/completed/cancelled; `urgency`: routine/semi-urgent/urgent) → doctor dashboard and patient dashboard both read real rows. **Correction (2026-08-17): the July 5 note below claiming doctor-side booking actions were "zero implementation" is stale/wrong** — `app/doctor-dashboard/BookingActions.tsx` + `lib/bookings/actions.ts`'s `updateBookingAssignedToSelf` Server Action are real and working: a doctor can confirm (from `requested`), complete (from `confirmed`), or decline (from `requested`/`confirmed`), status-gated per action, plus a notes field. Verified by direct code read, not by re-trusting this document. Doctor self-registration/profile self-service, however, is still genuinely unbuilt — no `/register/doctor` route, no `lib/doctor-profile/actions.ts` exist; doctor accounts remain manually provisioned only. **Browse pages: real as of 2026-08-17** — `/medconnect`, `/medglobaldb`, `/doctors`, `/specialists`, the doctor detail page, and `/book`'s doctor list all now read `lib/db/queries/doctors.ts`'s real query layer, falling back to the static mock array only when no database is configured. **State-machine build (2026-08-17):** per the detailed module spec (`docs/decision-log/0009-medconnect-state-machine-build-plan.md`, source: `MedByClick_01_MedConnect_RU.pdf`), a phased build landed the same day — `bookings.caseStage` (10-state pipeline, additive to the existing `status`), a real dual-route doctor/institution matching algorithm (`lib/matching/doctorMatch.ts`, unit-tested, not yet wired into a live page), an SLA-deadline + escalation/abandonment sweep (`lib/bookings/sla.ts` + `app/api/cron/sla-sweep`, daily per the Vercel Hobby plan's cron limit) with an admin-gated queue (`/coordinator`), and a cross-module transfer flag (Phase H). Document upload, signed second-opinion reports, and consilium mode landed as **schema + synthetic-data-only UI shells** (`app/dashboard/CaseChecklistShell.tsx`, `SecondOpinionReportShell.tsx`, `app/doctor-dashboard/ConsiliumShell.tsx`) — real clinical *content* storage for all three hits the same Legal & Compliance gate already blocking `medical_history_entries` above, so no real doctor-facing write path exists for any of them yet. Full detail and what's still open: the linked decision log's addendum. | `core`; doctor vetting standard (Medical Community, per T3) before doctors go live | Medical Community (vetting standard, doctor admission) |
