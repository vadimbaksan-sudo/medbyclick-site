    ...
    <div className="space-y-6">
      ...
      {view.kind === "error" && (
        <div className="bg-stone-100 border border-stone-200 rounded-xl p-5 space-y-3">
          <p className="font-semibold text-stone-800 text-sm">Something went wrong</p>
          <p className="text-sm text-stone-700 leading-relaxed">{view.message}</p>
          <button
            onClick={reset}
            className="text-xs underline underline-offset-2 text-stone-700 hover:text-stone-900"
          >
            Try again
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <textarea
              ...
              className="w-full border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-900 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent resize-none"
            />
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-900 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent"
              />
            </div>
            <div>
              ...
              <select
                ...
                className="w-full border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent bg-white"
              >
          ...
          <p className="text-xs text-stone-400 text-center">{t.coordinatorNote}</p>
        </form>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="border border-stone-100 rounded-xl p-4">
              <p className="text-xs text-stone-400 mb-1">Suggested specialty (for coordinator to confirm)</p>
              <p className="font-semibold text-stone-900">{view.response.summary.recommendedSpecialty}</p>
            </div>
            <div className="border border-stone-100 rounded-xl p-4">
              <p className="text-xs text-stone-400 mb-1">Queue priority (staff-facing, not a medical judgment)</p>
          ...
          {view.response.summary.clarifyingQuestions.length > 0 && (
            <div className="border border-stone-100 rounded-xl p-4">
              <p className="text-xs text-stone-400 mb-2">Your coordinator may also ask</p>
          ...
          <div className="flex gap-3">
            <button
              onClick={reset}
              className="flex-1 py-3 border border-stone-200 hover:border-stone-400 text-stone-700 rounded-xl text-sm font-medium transition-colors"
            >
              Start Over
            </button>
            <a
