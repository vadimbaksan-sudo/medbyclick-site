    ...
    <div className="border border-slate-200 rounded-2xl p-6 bg-white hover:border-amber-300 hover:shadow-md transition-all">
      <div className="flex items-start justify-between mb-4 gap-4">
        <div>
          ...
          <h3 className="font-bold text-slate-900 text-lg leading-tight">{destination.tagline}</h3>
        </div>
        <p className="text-xs text-slate-400 mt-0.5 flex-shrink-0 text-right whitespace-nowrap">{destination.travelTime}</p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-3 mb-4">
        {destination.hospitals.map((h) => (
          <div key={h.city} className="bg-slate-50 border border-slate-100 rounded-xl p-3">
            <p className="text-sm font-semibold text-slate-900">{h.city}</p>
            <p className="text-xs text-slate-500 mb-1.5">{h.hospital}</p>
            <div className="flex flex-wrap gap-1">
              {h.specialties.map((s) => (
                <span key={s} className="text-[11px] px-2 py-0.5 bg-white border border-slate-200 text-slate-600 rounded-full">
                  {s}
                </span>
              ))}
            </div>
      ...
      <p className="text-xs text-slate-500 mb-4">
        Languages: <span className="font-medium text-slate-700">{destination.languages.join(" / ")}</span>
      </p>
