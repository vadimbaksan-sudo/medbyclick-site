...
export default function DoctorCard({ doctor }: { doctor: Doctor }) {
  return (
    <Link
      ...
      className="group block border border-stone-100 rounded-2xl p-6 hover:border-amber-300 hover:shadow-md transition-all bg-white"
    >
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          ...
          <p className="text-sm text-stone-400 mt-0.5">{doctor.title}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {doctor.subspecialties.map((sub) => (
          <span key={sub} className="text-xs px-2 py-0.5 bg-stone-100 text-stone-600 rounded-full">
            {sub}
          </span>
        ))}
      </div>
