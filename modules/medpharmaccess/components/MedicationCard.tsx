  ...
  return (
    <div className="border border-stone-200 rounded-2xl p-6 bg-white hover:border-amber-300 hover:shadow-md transition-all">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <h3 className="font-bold text-stone-900">{medication.name}</h3>
          <p className="text-xs text-stone-400 mt-0.5">{medication.genericName}</p>
        </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        <span className="text-xs bg-stone-100 text-stone-600 px-2 py-0.5 rounded-full">{medication.category}</span>
      </div>

      <p className="text-xs text-stone-500 mb-1"><span className="text-stone-400">Indication:</span> {medication.indication}</p>
      <p className="text-sm text-stone-600 leading-relaxed my-3">{medication.description}</p>

      <div className="flex flex-wrap gap-1 mb-4">
        {medication.countries.map((c) => (
          <span key={c} className="text-xs bg-stone-50 border border-stone-200 text-stone-500 px-2 py-0.5 rounded-full">{c}</span>
        ))}
      </div>
